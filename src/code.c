...
void writeInitialConditions(char *input_file, vector *pos, int numparticles, vector box, int time) {
	FILE *ifile = fopen(input_file, "w");

	fprintf(ifile, "%d %d %lf %lf %lf", time, numparticles, box.x, box.y, box.z);

	int i;
	for(i=0;i<numparticles;i++){

		fprintf(ifile, "\n%lf %lf %lf", (pos)[i].x, (pos)[i].y, (pos)[i].z);
	}
...
void computeSystemEnergy(vector *pos, int numparticles, vector box, double *energy, double *virial)
{
	...
	*energy = 0;
	...
	if (overlap>0)
	{
		fprintf(stderr, "Error in initial condition. There are %d overlaps\n", overlap);
		exit(1);
	}
...
{
	//  ./build/start initial_conditions.dat 0.5 10000

	if (argc!=4)
	{
	printf("%s [initial file] [temperature] [iterations]\n",argv[0]);
	exit(1);
	}
	...
	while (++i<total_length)
	{
		...
		if (i % step == 0)
		{
			...
			sprintf(fname,"output_%d",i);
			writeInitialConditions(fname, pos, numparticles, box, i);
